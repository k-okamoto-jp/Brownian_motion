...
class Simulation:
    ...
    def init(self):
        ...
        return self.circles, self.numtim_text

    def animate(self, i):
        """The function passed to Matplotlib's FuncAnimation routine."""

        self.advance_animation(0.01)
        self.numtim_text.set_text('t = {:d}'.format(i) + '   n = {:d}'.format(self.n))
        return self.circles, self.numtim_text
    ...
    def do_animation(self, save=False):
        ...
        self.numtim_text = self.ax.text(0.02, 0.95, '', transform=self.ax.transAxes)

        anim = animation.FuncAnimation(fig, self.animate, init_func=self.init,
                               frames=1000, interval=2, blit=False)
        if save:
